...
if (
    ...
    $("#flashcard-search").on("keyup paste", function () {
        ...
        if (text != "") {
            let filteredData = fiszki_data.filter(data => {
                text = removeDiacritics(text);
                return (
                    removeDiacritics(data.first.toUpperCase()).includes(
                        text.toUpperCase()
                    ) ||
                    removeDiacritics(data.second.toUpperCase()).includes(
                        text.toUpperCase()
                    ) ||
                    removeDiacritics(data.name.toUpperCase()).includes(
                        text.toUpperCase()
                    )
                );
            });
            ...
            filteredData.forEach(el => {
                $(".fiszki-cat[data-name='" + el.name + "']").show();
            });
        } else {
            $(".fiszki-cat[data-name]").show();
        }
    });
    ...
    $.get("functions/Fiszki.php?method=pobierzFiszki&lang=" + lang, function (
        ...
        // JSON.parse(x).forEach(el => {
        //     if (typeof fiszki_data[el.name] == "undefined") {
        //         fiszki_data[el.name] = [];
        //     }
        //     fiszki_data[el.name].push(el);
        // });
    });
}
